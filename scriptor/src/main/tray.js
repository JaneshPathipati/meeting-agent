// file: scriptor/src/main/tray.js
...
function buildMenu() {
  ...
  const items = [
    { label: `Scriptor v${app.getVersion()}`, enabled: false },
    { type: 'separator' },
...
function setRecordingStatus(recording, onPauseResume) {
  ...
  if (recording) {
    ...
    if (tray) tray.setToolTip('Scriptor — Recording: <1 min');

    // Update tooltip every 60s with elapsed duration
    _durationInterval = setInterval(() => {
      if (tray && _recordingStartTime) {
        tray.setToolTip(`Scriptor — ${formatElapsed(_recordingStartTime)}`);
      }
    }, 60000);
  } else {
    _recordingStartTime = null;
    if (tray) tray.setToolTip('Scriptor');
  }
