// file: scriptor/src/detection/presenceDetector.js
...
// Retries every 30 minutes in case the license condition changes mid-session.
let _teamsLicenseAbsent = false;
let _teamsLicenseAbsentTime = 0;
const LICENSE_RETRY_INTERVAL_MS = 30 * 60 * 1000; // Retry every 30 minutes
...
async function checkTeamsPresence() {
  // Skip if we've confirmed this user has no Teams license (retry periodically)
  if (_teamsLicenseAbsent) {
    if (Date.now() - _teamsLicenseAbsentTime < LICENSE_RETRY_INTERVAL_MS) return null;
    _teamsLicenseAbsent = false; // Allow retry
    log.info('[Presence] Retrying Teams license check after cooldown');
  }
  ...
  try {
    ...
    if (status === 401 || status === 403) {
      ...
      log.info('[Presence] User has no Teams license (404) — disabling presence checks for 30 min');
      _teamsLicenseAbsent = true;
      _teamsLicenseAbsentTime = Date.now();
    }
