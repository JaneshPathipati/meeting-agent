// file: scriptor/src/audio/micRecorder.js
...
async function startMicRecording(outputPath) {
  ...
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => resolve(outputPath), 800);
  });
}
...
function stopMicRecording() {
  ...
  return new Promise((resolve) => {
    ...
    // Resolve when ffmpeg exits
    proc.on('close', done);

    // Safety timeout: if ffmpeg doesn't exit in 10s, force kill
    const timeout = setTimeout(() => {
      if (resolved) return;
      log.warn('[MicRecorder] ffmpeg stop timeout — forcing kill');
      try { proc.kill('SIGTERM'); } catch (_) {}
      // Give it another second to exit after SIGTERM, then resolve regardless
