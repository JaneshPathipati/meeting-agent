// file: scriptor/src/ai/localSummary.js
...
function callOpenAISummary(transcript, userName) {
  return new Promise((resolve, reject) => {
    ...
    const speakerContext = userName && userName !== 'You'
      ? `The employee being monitored is "${userName}". In the transcript they appear as "${userName}". ` +
        `Remote participants appear as "Remote Participant 1", "Remote Participant 2", etc. ` +
        `Always refer to all participants by their actual names throughout the summary.\n\n`
      : '';

    const systemPrompt =
      `You are a senior workplace meeting analyst. Analyze the transcript and return a JSON object with exactly these fields:\n` +
      `- category: one of "client_conversation","consultant_meeting","target_company","sales_service","general"\n` +
      `- summary: REQUIRED — a detailed markdown-formatted meeting summary (400-800 words) using this structure:\n` +
      `  ## Meeting Overview\n` +
      `  **Type:** [category in plain English]\n` +
      `  **Participants:** [list all speakers with roles if apparent]\n\n` +
      `  ## Key Discussion Points\n` +
      `  - **[Topic]:** [2-3 sentence detail with speaker attribution]\n\n` +
      `  ## Decisions Made\n` +
      `  1. [Decision with context] (or "No formal decisions were recorded.")\n\n` +
      `  ## Action Items\n` +
      `  | Owner | Task | Deadline |\n` +
      `  |-------|------|----------|\n` +
      `  | [name] | [task] | [deadline or TBD] |\n\n` +
      `  ## Key Takeaways\n` +
      `  - [2-4 high-level insights]\n\n` +
      `  ## Follow-Up Questions\n` +
      `  - [2-4 forward-looking questions]\n\n` +
      `- structuredJson: { executive_summary: string, participants: string[], key_discussion_points: string[], decisions_made: [{decision: string, context: string}], action_items: [{owner: string, task: string, deadline: string}], open_questions: string[] }\n` +
      `- toneAlerts: array of { start_time: "HH:MM:SS", speaker: string, severity: "low"|"medium"|"high", flagged_text: string, reason: string } for aggressive/unprofessional tone, empty array if none\n\n` +
      speakerContext +
      `QUALITY REQUIREMENTS:\n` +
      `- Be thorough and detailed. Always attribute statements to speakers by name.\n` +
      `- Use specific numbers, dates, and names from the transcript — do not generalize.\n` +
      `- The summary field must always contain a non-empty markdown string.\n` +
    ...
    req.setTimeout(90000, () => {
      req.destroy(new Error('OpenAI summary request timed out after 90s'));
    });
...
async function generateLocalAI(transcript, userName, meetingId) {
  ...
  const tmpFile = path.join(
    os.tmpdir(),
    `scriptor_tx_${meetingId || Date.now()}.json`
  );
