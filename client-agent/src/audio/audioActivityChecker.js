// file: client-agent/src/audio/audioActivityChecker.js
// Checks if system audio is actively flowing by reading the tail of the WAV file
// being written by audiocapturer.exe and computing RMS amplitude.
// This is a read-only operation — no interference with the recording process.
//
// Amplitude notes (from testing on this system):
//   - audiocapturer.exe raw capture: max amplitude ~0.05 (very low)
//   - After loudnorm normalization: ~0.8
//   - Digital silence: RMS = 0
//   - Background noise floor: RMS ~0.0005
//   - Quiet speech (raw): RMS ~0.002-0.01
//   - Normal speech (raw): RMS ~0.01-0.05
...
// Set very low because audiocapturer.exe captures at low amplitude (~0.05 raw max).
...
function hasAudioActivity(wavPath) {
  try {
    ...
    // File might be locked momentarily by audiocapturer — not an error
    log.debug('[AudioActivity] Check failed (likely file lock)', { error: err.message });
    return false; // On error, don't claim audio is active
  }
}
