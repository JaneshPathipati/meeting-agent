-- hotfix-admin-invite-code.sql
-- Fix: generate_admin_invite() stores code WITH dashes ('XXXX-XXXX-XXXX')
-- but redeem_admin_invite() receives code WITHOUT dashes from the frontend.
-- Solution: strip dashes on both sides during lookup so both formats match.
-- Also fixes generate_admin_invite() to store without dashes going forward (cleaner).

-- Fix 1: Patch the lookup in redeem_admin_invite() to strip dashes on both sides.
--        This fixes all existing codes in the DB (stored with dashes) AND future ones.
...
BEGIN
  ...
  -- Validate invite code — strip dashes from BOTH the stored code and the input
  -- so it works whether the DB has 'XXXX-XXXX-XXXX' or 'XXXXXXXXXXXX'
...
-- Re-grant permissions (REPLACE drops them)
...
-- Fix 2: Store codes WITHOUT dashes going forward (cleaner, consistent with how
--        the frontend sends them). Existing codes in the DB still work via Fix 1.
...
BEGIN
  ...
  -- Expire any previous unused invites for this org
  UPDATE admin_invites
  SET expires_at = NOW()
  ...
  -- Generate 12-char raw code (no dashes) — stored as-is
  FOR i IN 1..12 LOOP
    v_raw := v_raw || substr(v_chars, floor(random() * length(v_chars) + 1)::int, 1);
  END LOOP;

  -- Display format with dashes (returned to frontend for copy/paste UX only)
  v_display := substr(v_raw, 1, 4) || '-' || substr(v_raw, 5, 4) || '-' || substr(v_raw, 9, 4);

  v_expires_at := NOW() + INTERVAL '24 hours';

  -- Store WITHOUT dashes — consistent with what redeem receives after frontend strips them
  INSERT INTO admin_invites (org_id, code, created_by, expires_at)
  VALUES (v_org_id, v_raw, v_profile_id, v_expires_at);

  RETURN jsonb_build_object(
    'code', v_display,   -- return display format (with dashes) for the UI
